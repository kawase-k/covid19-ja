const CovidGateway = require('./covidGateway')
...
class OutputFormatter {
  ...
  async showResult () {
    if (this._answer === this._queryFirst) {
      ...
      const result = await this._gateway.fetchInpatientData()
      console.log(`${result.date}時点の${this._queryThird}は${result.ncures}人です。`)
    } else {
      const result = await this._gateway.fetchDeathData()
